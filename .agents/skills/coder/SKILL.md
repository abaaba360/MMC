...
- `state/agent_outputs/interface_contracts.json`（跨问输入输出契约）
- `state/agent_outputs/argument_map.json`（论证骨架）
- `state/agent_outputs/model_route_decision.json`（基线、核心模型与消融计划）
...
正式大规模编码前，先用代表性小样本、粗网格或合成数据实现最小可运行原型，检查量纲、约束方向、接口字段和结果方向。生成 `state/agent_outputs/prototype_review.json`；逻辑未闭合时回退模型设计，不得继续堆叠模块。
...
同时生成 `q{i}_claim_registry.json`。每条拟写入论文的声明必须登记 `claim_id`、对应 `requirement_id`、结果JSON路径、代码函数/符号、单位、适用范围、不确定性、图表、验证方法与 `verified` 状态。只有已验证声明可交给Writer。

### Step 4：生成论文级图表

图表按论证需要生成，不设固定数量。每张图必须绑定一个 `requirement_id` 或 `claim_id`，用于解释机制、主结果、基线对照或验证；没有明确问题的图不生成。
...
- [ ] 是否记录了运行环境和依赖版本？
- [ ] 是否输出最简单基线、核心模型和附加模块的消融结果？
- [ ] 每张图是否绑定了声明，且图后能写出明确结论？
...
- 图表数量服从证据需要；优先覆盖主结果、机制、基线对照和最脆弱假设验证，避免为凑类型而画图
